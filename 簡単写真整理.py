...
class PhotoApp:
    ...
    def add_preview(self, img, filepath, mode):
        tk_img = ImageTk.PhotoImage(self.resize_to_height(img.copy(), 300))
    ...
    def resize_to_height(self,img,target_height):
        ...
        return img.resize((int(img.width*(target_height/img.height)),target_height), Image.LANCZOS)
    ...
    def save_selected(self):
        ...
        if not selected_items:
            messagebox.showinfo("確認", "選択写真なし")
            return
        output_dir = filedialog.askdirectory(title="保存先フォルダ")
        if not output_dir: return
        threading.Thread(target=self._save_selected_thread, args=(selected_items, output_dir), daemon=True).start()
    ...
    def _save_selected_thread(self, selected_items, output_dir):
        ...
        for filepath, mode in selected_items:
            try:
                ...
                self.root.after(0, messagebox.showerror, "エラー", f"保存失敗: {os.path.basename(filepath)} ({mode})\n{e}")
        self.root.after(0, lambda: self.status_label.config(text=f"{saved_count}枚保存完了"))
        self.root.after(0, lambda: messagebox.showinfo("完了", f"{saved_count}枚保存しました"))
    ...
    def save_to_excel(self):
        ...
        if not selected_items:
            messagebox.showinfo("確認", "選択写真なし")
            return
        output_path = filedialog.asksaveasfilename(title="保存先Excel", defaultextension=".xlsx",
        ...
        for filepath, mode in selected_items:
            with Image.open(filepath) as img:
                ...
                tmp_path = os.path.join(os.getcwd(),"tmp_excel_img.png")
                img_to_save.save(tmp_path, format="PNG")
                xl_img = XLImage(tmp_path)
                ws.add_image(xl_img, f"A{row}")
                row += int(TARGET_HEIGHT_EXCEL / 20) + 1
        wb.save(output_path)
        messagebox.showinfo("完了", f"Excelに貼付しました: {output_path}")
    ...
    def open_settings(self):
        ...
        win.title("設定")
        frame = Frame(win,padx=10,pady=10)
        frame.pack()
        self.ratio_entries = {}
        ...
        btn_frame.pack()
        Button(btn_frame,text="保存",command=lambda:self.save_settings(win)).pack(side="left",padx=5)
        Button(btn_frame,text="デフォルトに戻す",command=self.reset_ratios).pack(side="left",padx=5)
        Button(btn_frame,text="キャンセル",command=win.destroy).pack(side="left",padx=5)
